...
public class HazardReportGeneratorTest {
	...
	private void initializeMockCauses() {
		...
		when(mockCause1.getTitle())
				.thenReturn("Improper feed system design leads to incorrect pressures or temperatures during USE run");
		when(mockCause1.getDescription()).thenReturn(
		...
		when(mockCause2.getDescription()).thenReturn(
		...
		when(mockCause3.getDescription()).thenReturn(
		...
		// when(mockCauseToCause.getTitle()).thenReturn("Excessive POGO creates
		// surges");
		when(mockCauseToCause.getDescription())
				.thenReturn("This cause is described in another hazard cause. Look it up.");
		...
		when(deletedCause.getDescription())
				.thenReturn("POGO causes excessive cavitation, leading to structural collapse.");
	...
	private void initializeMockControls() {
		...
		when(mockControl1.getDescription())
				.thenReturn("Numerous safety valves conforming to NASA-5713 will be in place around the tank.");
		...
		when(deletedControl.getDeleteReason())
				.thenReturn("This control is superceded by other controls and should be REMOVED.");
		when(deletedControl.getDescription())
				.thenReturn("This control has been deleted and shouldn't be displayed in the hazard report");
	...
	private void initializeMockVerifications() {
		...
		when(mockVerification1.getVerificationDesc())
				.thenReturn("Expansive Monte Carlo simulation of pressure distributions around rings 1, 2, and 5.");
		...
		when(mockVerification2.getVerificationDesc()).thenReturn(
				"This second verification contains no verification data other than the description. It is associated with all the controls in the hazard though.");
		...
		when(mockVerification2.getControls()).thenReturn(new Hazard_Controls[] { mockControlToCause });
		when(mockVerification2.getDeleteReason()).thenReturn(null);

		transferredVerification = mock(Verifications.class);
		when(transferredVerification.getVerificationNumber()).thenReturn(4);
		when(transferredVerification.getVerificationDesc())
				.thenReturn("The verification over there is relevant to me as well.");
		when(transferredVerification.getVerificationStatus()).thenReturn(null);
		when(transferredVerification.getVerificationType()).thenReturn(null);
		when(transferredVerification.getResponsibleParty()).thenReturn(null);
		when(transferredVerification.getEstCompletionDate()).thenReturn(null);
		when(transferredVerification.getControls()).thenReturn(new Hazard_Controls[] { mockControlToCause });
		when(transferredVerification.getDeleteReason()).thenReturn(null);
		when(transferredVerification.getTransfer()).thenReturn(23146);

		deletedVerification = mock(Verifications.class);
		when(deletedVerification.getVerificationNumber()).thenReturn(3);
		when(deletedVerification.getVerificationDesc())
				.thenReturn("This verification has been deleted and should not appear.");
		...
		when(deletedVerification.getControls())
				.thenReturn(new Hazard_Controls[] { mockControl1, mockControlToCause, mockControlToControl });
		when(deletedVerification.getDeleteReason()).thenReturn("Remove because it no longer applies.");

		when(mockControl1.getVerifications())
				.thenReturn(new Verifications[] { mockVerification1, mockVerification1, deletedVerification });
...
//		when(mockHazardService.getOrphanVerifications(mockHazard)).thenReturn(toReturn);
	...
	private void initializeTransfers() {
		...
		when(mockHazardCauseService.getHazardCauseByID(transferDestinationCause.getID()))
				.thenReturn(transferDestinationCause);
		...
		when(mockTransferService.getTransferByID(mockControlToCause.getTransfer()))
				.thenReturn(mockControltoCauseTransfer);
		...
		when(transferDestinationControl.getDescription())
				.thenReturn("There are many things that are involved in this control");
		...
		when(mockTransferService.getTransferByID(mockControlToControl.getTransfer()))
				.thenReturn(mockControlToControlTransfer);
		...
		Transfers mockVerificationToVerificationTransfer = mock(Transfers.class);
		when(mockVerificationToVerificationTransfer.getTargetID()).thenReturn(7654);
		when(mockVerificationToVerificationTransfer.getTargetType()).thenReturn("VERIFICATION");
		when(mockTransferService.getTransferByID(23146)).thenReturn(mockVerificationToVerificationTransfer);
		when(mockVerificationService.getVerificationByID(7654)).thenReturn(mockVerification1);
	}
